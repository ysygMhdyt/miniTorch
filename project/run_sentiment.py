...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        """
        x = embeddings.permute(0, 2, 1)  # [batch x embedding dim x sentence length]
        x1 = self.conv1(x).relu()
        x2 = self.conv2(x).relu()
        x3 = self.conv3(x).relu() # [batch x feature_map_size x sentence length]
        x = minitorch.max(x1, 2) + minitorch.max(x2, 2) + minitorch.max(x3, 2)
        x = self.linear(x.view(x.shape[0], self.feature_map_size))  # [batch x in_size]
